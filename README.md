...
## Overview
I have developed a series of [Ansible playbooks](https://docs.ansible.com/projects/ansible/latest/playbook_guide/playbooks_intro.html) to build web-hosting infrastructure comprising [Nginx](https://nginx.org) reverse-proxying to backend [Apaches](https://httpd.apache.org/) running PHP and Python-based applications.

![Diagram](schematic.png)

This project is primarily an investigation into the capabilities of Ansible automation and what can be achieved using Ansible playbooks for building web infrastructure using best-practices, etc.
...
- The back-end comprises [Apaches](https://httpd.apache.org/) having modules installed and configured to serve PHP or Python-based dynamic content.  
...
- The Nginx and Apache instances can also serve static content if desired, (the playbooks have [tagged](https://docs.ansible.com/projects/ansible/latest/playbook_guide/playbooks_tags.html) tasks to allow for content to be added from a Git repo).
- Strictly-speaking the Apaches could have been omitted from this infrastructure and back-end stand-alone application servers, such as PHP-FPM and Gunicorn, used instead but this was not the challenge here.  
- The Apache playbooks were also deliberately designed to allow for stand-alone installations of e.g. Apache/PHP-FPM and Apache/Python combinations for additional deployment options, (if for example you wanted to install such instances outside of a reverse-proxy infrastructure say).
...
- The `files/conf_*/` directories holds Nginx and Apache `.conf` files or `.j2` [Jinja2](https://pypi.org/project/Jinja2/) template files for creating custom `.conf` files.
...
- The `secrets.yml` file holds Git repo authentication details that is used by some of the playbooks (such as `playbook-nginx-rp.yml`).  This file needs to be encrypted using `ansible-vault` after updating with the authentication details.
...
- `playbook-python-mod_wsgi.yml` - installs `mod_wsgi` on a particular `web_backend` Apache host, creates a test Python web page, and then verifies it.
- `playbook-nginx-rp.yml` - installs Nginx as a front-end proxy.  Static web content is also installed from a Git repo using a Git access token, but tasks that do this are tagged, so can be omitted if desired.  The Git repo access token details must be placed in `secrets.yml` and encrypted using `ansible-vault`.  Consequently, this playbook must be given the password for the vault at run-time.
- `playbook-master.yml` - this is an optional playbook that runs all the above playbooks in the required order.
...
- [x] At least three Ansible Managed Nodes running RHEL-based O.S. (preferably RHEL v8 or v9).  Network connectivity between the Nginx node and backend Apache nodes should be allowed over ports TCP/80 and TCP/443.
...
- [x] Access to a Git repository holding your static and/or dynamic content.  The playbooks use GitLab with access either by using an access token or an SSH public key, but this can be changed with some minor modifications if required.

- [x] If downloading content from a Git repo using an SSH public key, the corresponding private key must exist on the host machine for the Ansible user (e.g. `~/.ssh/id_ed25519`).  Only the Apache web content is retrieved using this method.
...
Once the prequisites have been met, the project can be copied to the Ansible Control machine and the playbooks executed in the following (preferable) order using the `ansible-playbook <playbook_name>` command:
...
If desired, you can also run the playbooks selectively to create stand-alone web server hosting instances such as the following:

- If you just wanted an Apache web server that only serves static content, just run `playbook-apache.yml`.
- If you just wanted an Nginx web server that only serves static content, just run `playbook-nginx-rp.yml` (any reverse-proxy directives will need to be removed manually).
- If you just wanted an Apache web server that also serves PHP, run `playbook-apache.yml` followed by `playbook-nginx-rp.yml`.
...
- Investigate the possibility of creating an additional Ansible role e.g. `common` for performing common configurations needed across all hosts.
